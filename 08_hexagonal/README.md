...
[Driving Adapters]         [Application Core]        [Driven Adapters]
                           ┌──────────────┐
HTTP Controller ──port──►  │              │  ──port──► Postgres Repository
CLI Runner      ──port──►  │   Use Cases  │  ──port──► Stripe Gateway
Test Suite      ──port──►  │   + Entities │  ──port──► SendGrid Mailer
                           │              │  ──port──► Kafka Publisher
