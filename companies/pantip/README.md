...
By 2005, Pantip had millions of visitors:
  ...
  Peak traffic events:
    Thai New Year (Songkran) discussions → 10× normal traffic
  ...
  Forum-specific query:
    ...
    Same query, 500× different performance based on board
```
...
```
Write load: moderately sized (people write less than they read)
Read load: massive (everyone reads the same popular posts)

Solution: Primary + Read Replicas

[Primary MySQL] ──replication──► [Read Replica 1]
                ──replication──► [Read Replica 2]
                ──replication──► [Read Replica 3]

New post (write): → Primary MySQL
Read posts (read): → load balanced across 3 replicas

Benefit: 3× read capacity without sharding complexity
Drawback: replication lag (post appears 1-2 seconds after write)
          → acceptable for forum (not for financial transactions)
```
...
2019 Thai election day: political board traffic 100× normal
  → Cluster A at 95% CPU
...
Unexpected disaster (flood, earthquake):
  Traffic to news/disaster board: 50-100× in minutes
