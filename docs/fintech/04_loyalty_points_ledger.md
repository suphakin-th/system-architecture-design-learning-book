...
```mermaid
erDiagram
    USERS ||--o{ POINT_LOTS : earns
    POINT_LOTS ||--o{ POINT_TRANSACTIONS : "recorded on"
...
## Redeem: FIFO by Expiry, Not by Amount

```mermaid
flowchart LR
    subgraph Before["Before: redeem 70 pts"]
        L1["Lot 1 - 40 pts<br/>expires Jun"]
        L2["Lot 2 - 60 pts<br/>expires Sep"]
        L3["Lot 3 - 25 pts<br/>expires Feb"]
    end
    subgraph After["After: FIFO by expires_at"]
        A1["Lot 1 - 0 pts<br/>closed"]
        A2["Lot 2 - 30 pts<br/>expires Sep"]
        A3["Lot 3 - 25 pts<br/>untouched"]
    end
    L1 -- "drain fully (40)" --> A1
    L2 -- "drain partially (30 of 60)" --> A2
    L3 -. "not touched" .-> A3
```
...
```mermaid
sequenceDiagram
    participant A as User A (Lot 3, 25 pts, exp Feb)
    participant DB as point_lots / point_transactions
    participant B as User B

    A->>DB: transfer_out 20 (ref: transfer_789)
    DB-->>DB: Lot 3.remaining = 5 (still expires Feb)
    DB->>B: transfer_in 20 (new Lot 4)
    Note over B: Lot 4.expires_at = Feb - copied from Lot 3, not reset
```
...
```mermaid
flowchart TD
    C["Checkout at Store 2"] --> E["Look up point_redemption_scopes<br/>for this store"]
    E --> EL["Eligible types: A (network) + B (store-scoped)"]
    EL --> N["Normalize to value:<br/>remaining x value_per_point per lot"]
    N --> S{"Redemption strategy"}
    S -- "expiry-first (default)" --> X["Merge all eligible lots,<br/>sort by expires_at ascending,<br/>cut oldest-expiring value first regardless of type"]
    S -- "type-priority list" --> Y["Drain Type A lots fully (FIFO)<br/>before touching Type B"]
    X --> R["Redemption applied until target value is met"]
    Y --> R
```
...
| type | effect on lot | sign |
|---|---|---|
| `earn` | creates a new lot with amount = remaining | + |
| `redeem` | decrements remaining, FIFO by expires_at | - |
| `expire` | zeroes remaining once expires_at has passed | - |
| `transfer_out` | decrements sender's lot(s), FIFO by expires_at | - |
| `transfer_in` | creates a receiver lot, expires_at copied from source | + |
| `reverse` | restores remaining on the original lot_id from the ref'd transaction | + |
