...
The flow below shows how a real card number is exchanged for a useless-to-attackers token, so your application never stores the PAN.

```mermaid
flowchart TD
    A["Customer enters card: real PAN"] --> B["Tokenization Service (Stripe, Omise)"]
    B --> B1["Stores real PAN in their vault (PCI Level 1)"]
    B --> B2["Returns token: tok_abc123xyz"]
    B2 --> C["Your application stores: tok_abc123xyz"]
    C --> D["Your database NEVER sees the real card number"]
    C --> E["Your PCI scope MASSIVELY reduced"]
```

**When you need to charge again:** send `tok_abc123xyz` to Stripe. Stripe looks up the real PAN on its side and charges it. Your system never touches the real card number.

**Network tokenization (the next level up):**

- Visa Token Service / Mastercard Digital Enablement Service.
- Replaces the PAN with a network token that is specific to one merchant.
- Even if the token is stolen, it only works for that one merchant.
- Revocable: the token can be invalidated without reissuing the physical card.
- Used by Apple Pay, Google Pay, and Samsung Pay.
