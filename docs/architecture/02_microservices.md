...
A client hits the API Gateway, which routes to independent services. Each service owns its own database. Solid edges are synchronous REST/gRPC calls; dotted edges are async events over a message queue.

```mermaid
flowchart TD
    Client["Client"]
    GW["API Gateway<br/>auth, rate-limit, route, load-balance"]
    Client --> GW

    GW --> US["User Service<br/>domain, application, adapters, infrastructure"]
    GW --> OS["Order Service"]
    GW --> PS["Product Service"]
    GW --> PayS["Payment Service"]
    GW --> NS["Notification Service"]

    US --> UDB[("PostgreSQL: users DB")]
    OS --> ODB[("PostgreSQL: orders DB")]
    PS --> PDB[("MongoDB: products DB")]
    PayS --> PayDB[("PostgreSQL: payments DB")]
    NS --> NDB[("Redis: queued jobs")]
```

Services communicate two ways: solid `-->` is REST/gRPC (synchronous, immediate answer needed); dotted `-.->` is Events/MQ (async, fire-and-forget or eventual consistency).
...
The caller blocks and waits for a direct reply.

```mermaid
flowchart LR
    OS["Order Service"] -->|HTTP GET| US["User Service"]
```
...
The Order Service publishes an event and moves on; subscribers consume it independently. Dotted edges mark the async flow.

```mermaid
flowchart LR
    OS["Order Service"] -. publishes .-> EV["OrderPlaced event"]
    EV -. subscribes .-> PS["Payment Service"]
    EV -. subscribes .-> ES["Email Service"]
```
...
| **Memory** | High | N services x minimum heap = significant total RAM |
...
1. **Independent scaling** - scale Payment Service x10, keep User Service x1
...
|---|---|
| "Search needs 10x more CPU than checkout" | Scale each service independently |
