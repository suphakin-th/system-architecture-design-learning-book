...
1. **Write scalability** - 10 shards = 10x write throughput (each handles its own writes)
...
|---|---|
| "Our 50TB user table doesn't fit on the biggest RDS instance" | Split into 10 shards x 5TB each |
| "We're doing 80K writes/second but MySQL tops out at 20K" | 4 shards x 20K writes = 80K |
