...
Use ACID (PostgreSQL, MySQL with InnoDB) when:
  ✓ Financial transactions: money transfer, payment processing
  ✓ Inventory: can't oversell stock (check stock, decrement, or fail)
  ✓ User accounts: creating account must be atomic (profile + settings + preferences)
  ✓ Any operation where partial completion = corrupt state

Use BASE (Cassandra, DynamoDB, MongoDB default) when:
  ✓ Social media feeds: showing slightly old "like count" is fine
  ✓ Product recommendations: stale recommendations don't hurt the user
  ✓ Analytics data: approximate counts are acceptable
  ✓ Session data: if session data is briefly inconsistent, user just re-logs in
  ✓ Shopping cart: temporary inconsistency during heavy traffic is OK
```
...
Problems with 2PC:
  ...
  ✗ Latency: 2 round-trips to all participants before commit
  ✗ Not partition-tolerant: if network partition → participants wait forever
