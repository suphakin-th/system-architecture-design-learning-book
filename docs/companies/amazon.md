...
One Perl monolith (OBIDOS) held every concern and talked to a single Oracle database.

```mermaid
flowchart TD
    Browser["Customer Browser"]
    Browser --> OBIDOS["OBIDOS - Perl monolith"]
    OBIDOS --> Catalog["Product catalog logic"]
    OBIDOS --> Cart["Shopping cart logic"]
    OBIDOS --> Checkout["Checkout logic"]
    OBIDOS --> Reco["Recommendation logic"]
    OBIDOS --> Account["User account logic"]
    OBIDOS --> Fulfill["Order fulfillment logic"]
    OBIDOS --> DB["Oracle Database - one giant DB"]
```
...
- **Scaling all-or-nothing:** The product catalog gets 10x more traffic during holidays than the checkout - but both lived in the same process, so you scaled the whole thing
...
Traffic fans out from the load balancer to independent services, each owning its own datastore.

```mermaid
flowchart TD
    Browser["Customer Browser"]
    Browser --> ELB["ELB - Load Balancer"]
    ELB --> PCS["ProductCatalogService"]
    ELB --> SCS["ShoppingCartService"]
    ELB --> CS["CheckoutService"]
    ELB --> RS["RecommendationService"]
    ELB --> UAS["UserAccountService"]
    ELB --> FS["FulfillmentService"]
    PCS -->|products| Oracle["Oracle"]
    SCS -->|carts| Dynamo["Dynamo"]
    CS -->|orders| RDS1["RDS"]
    RS -->|reads| S3Dynamo["S3 plus DynamoDB"]
    UAS -->|users| RDS2["RDS"]
    FS -->|inventory| Aurora["Aurora"]
```

**Problems this solved:**
- Teams deploy on their own schedule
- ProductCatalog can scale x20 for holidays; Checkout stays at x1
...
Services are grouped into domains; each domain is owned end-to-end by a single team.

```mermaid
flowchart TD
    subgraph Shopping["Domain: Shopping"]
        PCS["ProductCatalogService"]
        SS["SearchService"]
        PS["PricingService"]
    end
    subgraph Orders["Domain: Orders"]
        SCS["ShoppingCartService"]
        CS["CheckoutService"]
        OMS["OrderManagementService"]
    end
    subgraph Fulfillment["Domain: Fulfillment"]
        IS["InventoryService"]
        WS["WarehouseService"]
        ShS["ShippingService"]
    end
    subgraph Payments["Domain: Payments"]
...
> "Communication overhead scales as O(n^2) with team size. 5 people = 10 communication paths. 10 people = 45 communication paths. 20 people = 190 paths. Beyond ~8 people, everyone's in meetings instead of coding. Two pizzas forces the right team size."
...
| Relational data | Aurora | MySQL-compatible; 5x faster; auto-replicated |
